...
CUiAutomationAndCondition::CUiAutomationAndCondition(IUIAutomationAndCondition *ac) : m_cRef(1), m_ptinfo(NULL), m_pCondition(ac) {}

CUiAutomationAndCondition::~CUiAutomationAndCondition()
{
...
IFACEMETHODIMP 
CUiAutomationAndCondition::QueryInterface(REFIID riid, void **ppv)
{
	HRESULT hr = S_OK;
	...
	else if (IsEqualIID(IID_IDispatchedUiAutomationAndCondition, riid))
	{
		*ppv = static_cast<IDispatchedUiAutomationAndCondition *>(this);
	}
	else
	{
		hr = E_NOINTERFACE;
...
CUiAutomationAndCondition::AddRef()
{
...
IFACEMETHODIMP_(ULONG) 
CUiAutomationAndCondition::Release()
{
...
IFACEMETHODIMP 
CUiAutomationAndCondition::GetTypeInfoCount(UINT *pctinfo)
{
	if (pctinfo == NULL)
	{
		return E_POINTER;
...
IFACEMETHODIMP 
CUiAutomationAndCondition::GetTypeInfo(UINT itinfo, LCID lcid, ITypeInfo **pptinfo)
{
	HRESULT hr = S_OK;
	*pptinfo = NULL;
...
HRESULT 
CUiAutomationAndCondition::LoadTypeInfo(ITypeInfo **pptinfo,
	const CLSID& libid,
	const CLSID& iid,
	LCID lcid)
{
...
IFACEMETHODIMP 
CUiAutomationAndCondition::GetIDsOfNames(REFIID riid,
	LPOLESTR *rgszNames,
	UINT cNames,
	LCID lcid,
	DISPID* rgdispid)
...
// of IDispatch::Invoke.
IFACEMETHODIMP 
CUiAutomationAndCondition::Invoke(DISPID dispidMember,
	REFIID riid,
	LCID lcid,
	WORD wFlags,
...
CUiAutomationAndCondition::GetBase(__RPC__deref_out_opt IUIAutomationCondition ** condition){
	*condition = m_pCondition;
	return S_OK;
}

HRESULT STDMETHODCALLTYPE
CUiAutomationAndCondition::GetBaseType( IID * idRef){
	*idRef = IID_IUIAutomationAndCondition;
	return S_OK;
}

#pragma endregion

#pragma region IUIAutomationAndCondition

HRESULT STDMETHODCALLTYPE 
CUiAutomationAndCondition::get_ChildCount(__RPC__out int *childCount){
	HRESULT hr = ((IUIAutomationAndCondition *)m_pCondition)->get_ChildCount(childCount);
	return hr;
}

HRESULT STDMETHODCALLTYPE 
CUiAutomationAndCondition::GetChildren(__RPC__deref_out_opt SAFEARRAY * *childArray){
	// XXX: We might have to massage the output to wrap it in variants
	HRESULT hr = m_pCondition->GetChildren(childArray);
	return hr;
}

#pragma endregion /* IUIAutomationAndCondition */